                ...
                <%if(request.getAttribute("listaelem")!=null){ %> 
             	<table class="table table-hover ">
            		...
            		<%ArrayList<entity.Elemento> listaElem = (ArrayList<entity.Elemento>)request.getAttribute("listaelem");
           			...
           			%>
           			<form class="form-persona" name="modifica" action="EliminaElemento" method="post">
                  	<input class="form-control" type="text" name="idE" id="idE" value="<%=e.getId()%>" hidden>
                  	<input class="form-control" type="text" name="nom" id="nom" value="<%=e.getNombre()%>" hidden>
                  	<tr> 
            			<th><%=e.getTipoElem().getDescripcion()%></th> 
            			<td><%=e.getNombre()%></td> 
            			<td><%=e.getDescrip()%></td> 
            			<td><button type="submit" class="btn btn-outline btn-danger btn-xs">Eliminar</button></td>
            		</tr>
            		</form>
            		<%} %>
            		</tbody>
