...
public class DatosPersona  
{
	...
	public ArrayList<Persona> buscarTodo() throws Exception,SQLException
	{
		...
		try 
		{
			...
			throw new AppDataException(exc,"No es posible buscar una persona en la base de datos");
		}catch (Exception ex){
			throw ex;
		}
	...
	public ArrayList<Persona> buscarUsuariosExternos() throws Exception,SQLException
	{
		...
		try 
		{
			pstm = FactoryConexion.getInstancia().getConn().prepareStatement("select id,dni,nombre,apellido,usuario,contrasena,habilitado,categoria from personas where categoria = ?");
			...
			throw new AppDataException(exc,"No es posible buscar una persona externa en la base de datos");
		}catch (Exception e) {
			throw e;
		}
	...
	public void agregarPersona (Persona pers) throws Exception,SQLException
	{
		...
		try {
			...
			throw new AppDataException(e,"No se puede dar de alta, ya existe el usuario");
		}
	...
	public void eliminarPersona(Persona perselimina) throws Exception,SQLException
	{
		...
		try {
			...
			throw new AppDataException(exc,"Imposible eliminar,  la Persona posee reservas");
		}  
		catch (SQLException exc) {
			throw new AppDataException(exc,"Imposible eliminar,  la Persona posee reservas");
		} 
	...
	public void modificarPersona(Persona persmodifica) throws Exception,SQLException
	{
		...
		try {
			...
			throw new AppDataException(exc,"No es posible modificar una persona en la base de datos");
		}catch (Exception e){
			throw e;
		}
	...
	public Persona buscarPersonaPorUsuyClave(Persona pers) throws Exception,SQLException
	{
		...
		try {
			pstm = FactoryConexion.getInstancia().getConn().prepareStatement("SELECT * FROM personas WHERE usuario=? AND contrasena=?");
	...
	public Persona getPersona(Persona pers) throws Exception,SQLException
	{
		...
		try {
			pstm = FactoryConexion.getInstancia().getConn().prepareStatement("SELECT * FROM personas WHERE id=?");
